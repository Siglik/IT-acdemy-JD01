...
public class MatrixColumnsSorter {
	...
	Node currentNode;
	int[] order = null;
	// for all columns or until didn't get a result (order != null) try to
	...
	if (order != null) // check if order is null
	{
	    ...
	    // if order isn't - reorder matrix
	    matrix = MatrixSorterI.reorderMatrix(matrix, order);
	    MatrixSorterI.printMatrix(matrix);
	} else {
	    // if order is null reorder is impossible
	    System.out.print("Указанную матрицу невозможно упорядочить согласно условиям задачи. (done in "
		    + (System.currentTimeMillis() - start) + " ms)\n");
	}
    }
     ...
     * 
     * @param currentNode
     *            - currentNode - node of paths tree, where from to start search
     *            further
     * @return if find a full path - return a new order as a int array,<br>
     *         else return null
     */
	...
	for (int column : currentNode.availableColumns) {
	    /*
	     * find in all unused columns elements that lower than current
	     * element value and store it as child nodes to currentNode
	     */
		...
		// if last found node isn't in last row
		if (currentNode.branches[childNodesCounter].rowId != matrix.length - 1) {
		    // call recursive this child nodes
		    order = findDiagonal(currentNode.branches[childNodesCounter]);
		    if (order != null) // when get path - return
		    {
			return order;
		    }
		} else {
		    // if last found node is in last row - we reached the end
		    // so just return the path
		    return currentNode.branches[childNodesCounter].getPath();
		}
	    }
	}
	// if didn't reach the last row of the matrix than - path is didnt exist
	// - returning null
	return null;
    }
}