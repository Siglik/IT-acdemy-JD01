 ...
 */
	static class Test{
		...
		static void fillProductsDB(ProductsDB products){
			...
			/* add product by another way - using "copy" constructor and than change some fields*/
		...
		static void performTests(ProductsDB products){
			...
			System.out.println("\n\nФильтруем по товару \"" + search + "\" и цене не выше " + maxPrice + "руб. (найдено " + found +" товара(ов)):");
			products.printDBTableHeader();  //print result
			products.printProducts();
			
			/*reset filters and filter by best before date */
			products.clearFilters();
			Calendar cal = Calendar.getInstance();  //use Calendar class to make Date
			cal.add(Calendar.MONTH, 6);
			found = products.filterBestBefore(cal.getTime());
			System.out.printf("%n%nФильтруем по сроку годности не ранее %1$td.%1$tm.%1$ty  (найдено %2$d товара(ов)):%n",  cal.getTime(), found);
			products.printDBTableHeader();  //print result
			products.printProducts();
			...
			maxPrice = 2.0;  
			double minPrice = 1.0;
			cal.set(2016, Calendar.SEPTEMBER, 11);   //use Calendar class to make Date
			Date firstDate = cal.getTime();
			cal.set(2017, Calendar.SEPTEMBER, 11);
			Date secondDate = cal.getTime();
			products.filterBestBefore(firstDate, secondDate);   //set best before range
			found = products.filterByPrice(minPrice, maxPrice); //set price range and save count
			System.out.printf("%n%nФильтруем по сроку годности от %1$td.%1$tm.%1$ty  до %2$td.%2$tm.%2$ty  "
					+ "и по цене от %3$.2f до %4$.2f (найдено %5$d товара(ов)):%n",  firstDate, secondDate, minPrice,
					maxPrice,  found);
			products.printDBTableHeader();  //print result
			products.printProducts();	
		}
