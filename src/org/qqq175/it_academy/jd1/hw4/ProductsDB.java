...
c) список товаров, срок хранения которых больше заданного. */
	...
	/**
	 * Add new product to list (by copying all filed except id from excited product);
	 * @param product - some Product object ot copy
	 * @return last added product id
	 */
	...
	public void printDBTableHeader(){
		System.out.printf("%1$4s | %2$-20s | %3$-12s | %4$-20s | %5$-7s | %6$-8s | %7$-6s%n", "id", "Наименование", "UPC", "Производитель", "Цена", "Срок", "Кол-во");
		System.out.printf("%1$4s | %2$-20s | %3$-12s | %4$-20s | %5$-7s | %6$-8s | %7$-6s%n", "", "", "", "", "", "годости", "");
		System.out.println("-----------------------------------------------------------------------------------------------");
	}
	...
	public  int filterByName(String name){
		ArrayList<Product> result = new ArrayList<Product>();  //create new array
		for(Product product : filtredProducts){
			if(product.getName().equals(name)){   //for each element check for matching condition and save to result
				result.add(product);
			}
		}
	 ...
	 * @param maxPrice - max price or -1 for no limit by maxPrice
	 * @return count of elements matching condition
	 */
	public  int filterByPrice(double minPrice, double maxPrice){
		ArrayList<Product> result = new ArrayList<Product>();  //create new array
		for(Product product : filtredProducts){
			if(minPrice <= product.getPrice()  && (product.getPrice()  <= maxPrice || maxPrice == -1.0) ){   //for each element check for matching condition and save to result
				result.add(product);
			}
		}
		filtredProducts = result; 
	 ...
	 * @param minBestBefore - first date
	 * @param maxBestBefore - last date
	 * @return count of elements matching condition
	 */
	public  int filterBestBefore(Date minBestBefore, Date maxBestBefore){
		ArrayList<Product> result = new ArrayList<Product>();  //create new array
		for(Product product : filtredProducts){
			if(minBestBefore.before(product.getBestBefore())){   //for each element check for matching condition and save to result
