...
public class WordDeleter {
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter word length which are deleting:");  //input length
		...
		if (wordLength >0)  //check length
		{
			...
			for(int i = 0; i < strBuilder.length() ; i++){
				if (Character.isSpaceChar(strBuilder.charAt(i)) || ".,;:?!\"\'".contains(strBuilder.subSequence(i, i+1))){   ///check if char is space or punctuation
					if (currentIndex == wordLength && isConsonantLetter(strBuilder.charAt(i-currentIndex))){   //check word length and first letter
			...
			if (currentIndex == wordLength && isConsonantLetter(strBuilder.charAt(strBuilder.length() - currentIndex))){  //check and delete the last word
