 ...
 */
	public static void main(String[] args) {
		...
		for(int i = 0; i < someString.length(); i++) //for all chars in the string
		{
			...
			if (countCharInString(someString, someString.charAt(i)) > 1)
			{
				...
				System.out.println("Symbol " + someString.charAt(i) + " is repeat " + countCharInString(someString, someString.charAt(i)) + " times in the string.");
				break;
			}
		}
		/* print result */
	...
	public static int countCharInString(String inputString, char inputChar)
	{
		int count = 0;
		int index = 0;
		index = inputString.indexOf(inputChar, index); //try to find first inoutChar
		while (index != -1){	//if found
			count++;	//increment count
			index = inputString.indexOf(inputChar, index+1);	//try to find next
		}
