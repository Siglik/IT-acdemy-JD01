...
/**
 * String check if some sting has same set and amount of characters to another<br>
 *  <i>Exercise 6</i>
 * @author qqq175
 *
 */
public class StingDemixer {
	...
	public static void main(String[] args) {
		...
		if (someString.length() != anotherString.length()) {  //if length is not equals - string are different
			srtingsAreMixEquals = false;
		} else { //lengths are same
			for (int i = 0; i < someString.length(); i++) {
				/*
				 *Check if letter[i] contains in both strings and how many times 
				 */
				if (IsCharUniqeChecker.countCharInString(someString, someString.charAt(i)) != IsCharUniqeChecker
					...
					srtingsAreMixEquals = false; //if count isnt same - set result false and break
					break;
				}
			}
		}
